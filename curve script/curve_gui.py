...
A tiny drag-and-drop GUI in front of :mod:`curve_extractor`.

Drop a Studio 5000 ``.ACD`` file (or later an ``.L5K`` export) onto the window
and it lays the fuel/air curve out as a table:

    corner "gas", then rows: purge, LtOff, 1..16
    columns: Air, Fuel Act1, FGR, VFD, and O2 (only when DesiredO2.Cfg.O2Curve = 1)
...
class CurveApp:
    ...
    # -- rendering ----------------------------------------------------------
    def _show_result(self, data: ce.CurveData):
    ...
    def _render_table(self, table: dict):
        ...
        self.tree.column("__label__", width=90, anchor="center")
        ...
        for row in table["rows"]:
            ...
            tag = "section" if row["label"] in ("purge", "LtOff") else ""
