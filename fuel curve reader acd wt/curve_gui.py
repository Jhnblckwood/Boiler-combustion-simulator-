...
class CurveApp:
    ...
    def _load(self, path: str):
        if not os.path.isfile(path):
            ...
            return
        is_acd = path.lower().endswith(".acd")
        self._set_status(f"Reading {os.path.basename(path)} …")
        self.drop.configure(text="\n  Working…\n")
        if is_acd:
            self._show_decrypt_popup()   # ".ACD" takes a moment to unpack
    ...
    def _worker(self, path: str):
        try:
            ...
            if ext == ".l5k":
                ...
                data = acd_reader.extract_multifuel_acd(path)
            else:
    ...
    # -- "Decrypting ACD" popup --------------------------------------------
    def _show_decrypt_popup(self):
        self._close_decrypt_popup()
        ...
        tk.Label(top, text="  Decrypting ACD…  ", bg="#eef3f8", fg="#33475b",
                 font=("Segoe UI", 12, "bold"), pady=18, padx=24).pack()
        bar = ttk.Progressbar(top, mode="indeterminate", length=220)
        bar.pack(padx=24, pady=(0, 18))
        bar.start(12)
        # center over the main window
        self.root.update_idletasks()
        x = self.root.winfo_rootx() + (self.root.winfo_width() - 270) // 2
        y = self.root.winfo_rooty() + (self.root.winfo_height() - 120) // 2
        top.geometry(f"+{max(x, 0)}+{max(y, 0)}")
        self._decrypt_popup = top

    def _close_decrypt_popup(self):
        top = getattr(self, "_decrypt_popup", None)
        if top is not None:
            try:
                top.destroy()
            except Exception:  # noqa: BLE001
                pass
            self._decrypt_popup = None
    ...
    def _show_result(self, data):
        if isinstance(data, fc.MultiFuelData):
            ...
            self._set_status(
                f"{data.source_file}  •  controller: {who}  •  both fuels"
            )
