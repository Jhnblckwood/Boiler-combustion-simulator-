...
public class ValveProvingLogic : BaseNetLogic
{
    ...
    private IUAVariable lgpVar, hgpVar, interlockVar;
    ...
    private Ellipse lgpLed, hgpLed, ledInterlock;
    ...
    private Button interlockButton, simInterlockButton;
    ...
    public override void Start()
    {
        ...
        {
            StartInternal();
            Log.Info("ValveProvingLogic", "ValveProvingLogic BUILD v10 (interlock) started OK");
        }
        catch (Exception ex)
        {
    ...
    private void StartInternal()
    {
        ...
        simInterlockButton = Owner.Get<Button>("SimInterlockButton");
    ...
    private void RunSequence(bool auto)
    {
        ...
        // The interlock string must stay closed at all times: if it opens
        // at any point in the sequence or the run, lock out immediately.
        if (!ReadBool(interlockVar))
        {
            if (step != Step.Standby)
            {
    ...
    private void UpdateGraphics(bool auto)
    {
        ...
        SetText(tagLabel4, interlockClosed ? "INTERLOCK - CLOSED" : "INTERLOCK - OPEN");
        SetText(interlockButton, interlockClosed ? "INTERLOCK: CLOSED" : "INTERLOCK: OPEN");
        SetText(simInterlockButton, interlockClosed ? "SIM OPEN INTERLOCK" : "SIM CLOSE INTERLOCK");
        ...
        startButton.Enabled = step == Step.Standby && !v1 && !v2 && !pilotLit && interlockClosed;
        vp1Button.Enabled = !auto;
        vp2Button.Enabled = !auto;
        pilotButton.Enabled = !auto;
        // The MANUAL interlock control follows the other manual controls;
        // the SIM button can break the string in either mode.
