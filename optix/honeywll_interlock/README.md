# Valve Proving Honeywell + Interlock — FactoryTalk Optix Project
...
`ValveProvingHoneywellInterlock.optix`.
...
- **Two controls**, as requested:
  ...
  - **SIMULATION ▸ SIM OPEN INTERLOCK** — breaks (or re-closes) the string
    in either mode, so you can drop the interlock mid-run and watch the
    lockout.
...
```
optix/honeywll_interlock/
```

### 3. Open the project in Optix Studio

1. Start **FactoryTalk Optix Studio**.
2. **Open project** → browse into `optix/honeywll_interlock/` and pick
   **`ValveProvingHoneywellInterlock.optix`** (or just double-click that file).
...
2. Watch the Output pane — it must end with a successful build of
   `ValveProvingHoneywellInterlock` / `ValveProvingLogic.cs` with **0 errors**.
...
3. **Verify the right build is running** — open the runtime log
   ...
   ```
   ValveProvingLogic BUILD v10 (interlock) started OK
   ```
...
2. The **INTERLOCK starts closed** (green light). Press **SIM OPEN
   INTERLOCK** and START BURNER greys out — nothing can begin until it is
   closed again.
...
| **SIM OPEN INTERLOCK** in standby, then press START | Start refused (button disabled); forcing it → `LOCKOUT 19` |
| **SIM OPEN INTERLOCK** any time during the sequence or run | Immediate `LOCKOUT 19` — interlock string broke |
...
| Same error persists after a “fix” | Stale DLL — confirm the `BUILD v10 (interlock)` marker; if old: close Studio, delete `bin/ obj/ .vs/`, rebuild |
...
| `Interlock` | Boolean | **Burner interlock string INPUT — TRUE = closed. Nothing starts unless closed; opening at any time locks out (code 19)** |
...
```
ValveProvingHoneywellInterlock.optix          project root (open this)
ValveProvingHoneywellInterlock.optix.design   Studio design companion
...
ProjectFiles/NetSolution/            C# solution
  ValveProvingLogic.cs               sequence + all animation (BUILD v10 (interlock))
